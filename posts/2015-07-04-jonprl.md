...
(including homebrew I've heard?) otherwise just grab the binary from
the [website][nj]. After this the following commands should get you a
working executable
...
basic forms. We can apply an "operator" (I'll clarify this in a
...
`spread` which let's us provide a term which depends on the two
components of a pair. Eg `spread(0; 2)`, you give it a `Σ` in the
first spot and `x.y.e` in the second and it'll replace `x` with the
first component and `y` with the second. Can you think of how to write
`fst` and `snd` with this?
