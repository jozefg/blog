...
*JonPRL switched to ASCII syntax so I've updated this post
 accordingly*

I was just over at OPLSS for the last two weeks. While there I finally
met [Jon Sterling][jon] in person. What was particularly fun is that
...
basic forms:
 ...
 - And we have abstraction with `x.e`. JonPRL has one construct for
   binding `x.e` built into its syntax, that things like `lam` or
   `fun` are built off of.

An operator in this context is really anything you can imagine having
a node in an AST for a language. So something like lam is an operator,
...
with lam has an arity is written `(1)` since it takes 1 argument which
...
write `(lamx.x) y` in JonPRL as `ap(lam(x.x); y)`. The type of
functions is written with `fun`. Remember that JonPRL's language has a
notion of dependence so the arity is `(0; 1)`. The construct `fun(A;
x.B)` corresponds to `(x : A) → B` in Agda or `forall (x : A), B` in
Coq.

We also have dependent sums as well (`prod`s). In Agda you would write
`(M , N)` to introduce a pair and `prod A lam x → B` to type it. In JonPRL
you have `pair(M; N)` and `prod(A; x.B)`. To inspect a `prod` we have
`spread` which let's us eliminate a pair pair. Eg `spread(0; 2)` and
you give it a `prod` in the first spot and `x.y.e` in the second.  It'll
...
were lam or something! I call them types because I'm thinking of later
...
see `fun(unit; _.unit)` in a language without types, so I wanted to make
this explicit.
...
constants.
 ...
 - `U{i}`, the ith level universe used to classify all types that can
   ...
   terms like `fun` and it contains all the types of smaller universes
 ...
 - `member(0; 0)` this is just like `=` but internalizes membership in a
...
the judgment and member the term. There's nothing inherent in `member` above
...
we're looking at a function, so something like  `H ⊢ fun(A; x.B)`, this
...
prod? Well now things get a bit dicey. We'd might expect to get two
subgoals if we run `intro` on `H ⊢ prod(A; x.B)`, one which proves
...
proving that `· ⊢ prod(unit; x.unit)`, we run `intro [<>]` which gives
us two subgoals `· ⊢ member(<>; unit)` and `· ⊢ unit`. Here the `[]`
let us denote the realizer we're passing to `intro`. In general any
term arguments to a tactic will be wrapped in `[]`s. So the first goal
...
given realizer into the second argument of `prod`, `x.unit`, and asks
us to prove that. Notice how here we have to prove `member(<>; unit)`?
This is where that weird `member` type comes in handy. It let's us
sort of play type checker and guide JonPRL through the process of type
checking. This is actually very crucial since type checking in Nuprl
and JonPRL is undecidable.

Now how do we actually go about proving `member(<>; unit)`? Well here
`mem-cd` has got our back. This tactic transforms `member(A; B)` into the
...
a normal equivalence relation! So in JonPRL `member` is actually just
a very thin layer of sugar around `=` which is really the core
...
since writing `prod(A; x.B)` is kind of annoying. We do this using
operator
...
`prod` is sugar for `prod`. To do this we use `=def=` which gives us a way
...
Now we can change any occurrence of `prod(A; B)` for `prod(A; _.B)` as
...
`A = B`! In another example, `lam(x. x) ∈ fun(A; _.A)` for all `A`!  This
...
`lam_. lam_. spread(_; s._.s)`! This is probably closer to what you've
...
behaves a lot like `fun` but with one big difference, the definition of
...
There's no spurious `lam _. ...` at the beginning! Even more wackily, we
